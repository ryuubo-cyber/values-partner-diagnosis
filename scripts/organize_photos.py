...
def _copy_large_file(src, dst):
    """大きなファイルでもタイムアウトしないよう手動でコピーする"""
    with open(src, "rb") as fsrc, open(dst, "wb") as fdst:
        while True:
            chunk = fsrc.read(1024 * 1024)  # 1MBずつ
            if not chunk:
                break
            fdst.write(chunk)
    # メタデータ（更新日時など）もコピー
