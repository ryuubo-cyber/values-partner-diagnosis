...
- 【既婚者の場合】マッチングアプリ・恋愛系の出会いは一切推奨しない。代わりにビジネスマッチング（LinkedIn・Yenta等）、友人・人脈づくり、趣味コミュニティ、家族ぐるみの交流を提案する。タイトルは「人脈づくりのヒント」にする
- 【未婚者の場合】マッチングアプリを最優先で推奨する（母数が数百万人で効率的）
- 年代別の具体的なアプリ戦略を含める
- 職場・既存の人間関係からの発展にも言及
- 【過去の出会い方（meetingHistory）がある場合】既に試した出会い方をそのまま推奨しない。「まだ試していない方法」を優先的に提案する
- 【パートナーとの出会いきっかけ（partnerMeetingWay）がある場合】その経験を踏まえたアドバイスにする
...
- idealPartnerAnalysis は具体的だが端的に（150字程度）${data.profile.futurePlan ? `。将来設計「${data.profile.futurePlan}」を踏まえたパートナー像を描く` : ""}${data.profile.friendCount ? `。友人関係「${data.profile.friendCount}」の傾向も考慮` : ""}
...
- moneyAnalysis は100字程度${data.profile.financialHabit ? `。お金の使い方「${data.profile.financialHabit}」とスコアの関連を分析` : ""}
- loveAndMarriageAnalysis は100字程度${data.profile.familyStructure ? `。家族構成「${data.profile.familyStructure}」を踏まえる` : ""}${data.profile.futurePlan ? `。将来設計「${data.profile.futurePlan}」を踏まえた恋愛・結婚観を分析` : ""}
- regionalCompatibility は150字程度。${data.profile.birthPlace ? `出身地「${data.profile.birthPlace}」の県民性を分析し、` : ""}暮らし方（都会/田舎）の相性や、価値観に合う地域特性を具体的に。「どこの県が合う」より「どんな地域特性の人と合うか」を重視${data.profile.foodPreference ? `。食の傾向「${data.profile.foodPreference}」も地域性と絡めて分析` : ""}
...
- プロフィール情報（趣味・性格タイプ・リテラシー・美容関心・部活経験・移動手段・友人との付き合い方・お金の使い方・食の傾向・将来設計など）はすべてJSON内に含まれている。各セクションでこれらの情報を活用してパーソナライズすること
- 【重要】友人との付き合い方（friendCount）・お金の使い方（financialHabit）・食の傾向（foodPreference）・将来設計（futurePlan）は複数選択式データ。それぞれの組み合わせから、このユーザー固有の傾向を読み取ること
