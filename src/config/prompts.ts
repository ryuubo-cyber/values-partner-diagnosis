...
同じスコアパターンの人が100人いても、プロフィールの違い（趣味、性格タイプ、ITリテラシー、マネーリテラシー、美容関心度、移動手段、部活経験など）によって100通りの異なる結果が出るようにしてください。
また、同じ人が再生成しても、異なる切り口・比喩・表現で別の文章を生成してください。

【重要】100問の回答スコアは各カテゴリの合計点だけでなく、個別の質問への回答傾向（高得点カテゴリ内でも特にどの質問に強く反応しているか、逆転項目の回答パターン等）をAIとして推測し、そのユーザーの「こだわりの質」まで分析してください。単なる点数の高低ではなく、「なぜそのスコアになったか」の背景をプロフィール情報と組み合わせて読み解き、世界に一つだけの診断結果にしてください。
...
${data.profile.hobbies ? `- 趣味は「${data.profile.hobbies}」。趣味の組み合わせから見える人物像や、同じ趣味を持つパートナーとの相性にも言及する` : ""}
${data.profile.personalityType ? `- 性格タイプは「${data.profile.personalityType}」。このタイプが恋愛・仕事・人間関係に与える影響を分析に組み込む` : ""}
${data.profile.clubActivity ? `- 過去の部活・スポーツ経験は「${data.profile.clubActivity}」。この経験が現在の価値観形成にどう影響しているかを考察する` : ""}
${data.profile.beautyInterest ? `- 美容への関心は「${data.profile.beautyInterest}」。パートナーとの美意識の相性にも言及する` : ""}
${data.profile.itLiteracy ? `- ITリテラシーは「${data.profile.itLiteracy}」。デジタルツールの活用度がパートナーシップや出会い方にどう影響するか分析する` : ""}
${data.profile.moneyLiteracy ? `- マネーリテラシーは「${data.profile.moneyLiteracy}」。資産形成への取り組み度合いとmoneyスコアの関連を分析し、パートナーとの経済的相性に反映する` : ""}
${data.profile.transportation ? `- 主な移動手段は「${data.profile.transportation}」。行動範囲やライフスタイルとの関連で出会いのヒントに活用する` : ""}
- 各分析セクションでは、プロフィール情報（趣味・性格タイプ・リテラシー・美容関心・部活経験など）とスコアの「掛け算」で、このユーザーにしかない独自の洞察を生み出す。誰1人同じ結果にならないように
